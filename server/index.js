...
async function sendWhatsAppMedia(phone, mime, base64) {
  if (!WA_API_TOKEN || !WA_INSTANCE_ID) {
    console.log('WA API credentials not set, skipping media send');
    return;
  }
  const url = `https://waapi.app/api/v1/instances/${WA_INSTANCE_ID}/client/action/send-media-from-base64`;
  try {
    const res = await fetch(url, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({ phone, mimeType: mime, base64 }),
    });
    const data = await res.text();
    console.log('WA API media response', data);
  } catch (e) {
    console.error('WA API media error', e.message);
  }
}
...
app.post('/messages/:userId', (req, res) => {
  ...
  const insertOutgoing = (convId) => {
    db.run(
      ...
      [convId, userId, 'outgoing', message, media?.mimetype || null, media?.data || null],
      function (err) {
        if (err) return res.status(400).json({ error: err.message });
        db.run(
        ...
        if (media?.data) {
          sendWhatsAppMedia(phone, media.mimetype, media.data);
        }
